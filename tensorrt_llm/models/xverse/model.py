...
from ...layers import (MOE, Attention, AttentionMaskType, ColumnLinear,
                       Embedding, GatedMLP, MLP, PositionEmbeddingType, RmsNorm)
...
class XverseDecoderLayer(Module):
    ...
    def __init__(self, config: XverseConfig, layer_idx: int):
        ...
        if config.num_shared_experts != 0:
            ClsMLP = MLP
            self.shared_experts = ClsMLP(hidden_size=config.hidden_size,
                                         ffn_hidden_size=mlp_hidden_size * config.num_shared_experts,
                                         hidden_act=config.hidden_act,
                                         dtype=config.dtype,
                                         bias=config.mlp_bias,
                                         tp_group=config.mapping.tp_group,
    ...
    def forward(self,
        ...
        self.register_network_output(f'input', hidden_states)
        ...
        self.register_network_output(f'attn_output', attention_output[0])
        ...
        if self.shared_experts is not None:
            ...
            hidden_states = self.mlp(hidden_states, lora_layer_params=lora_layer_params) + shared_hidden
            self.register_network_output(f'shared_expert_output', shared_hidden)
        else:
            hidden_states = self.mlp(hidden_states,
                                        lora_layer_params=lora_layer_params)
        self.register_network_output(f'mlp_output', hidden_states)
...
class XverseModel(Module):

    def __init__(self, config: XverseConfig) -> None:
        super().__init__()
        ...
        self.layers = DecoderLayerList(XverseDecoderLayer, config)
...
class XverseForCausalLM(DecoderModelForCausalLM):
    config_class = XverseConfig

    def __init__(self, config: XverseConfig):
        transformer = XverseModel(config)
    ...
    def from_hugging_face(
            ...
            **kwargs):
        ''' Create a XverseForCausalLM object from give parameters
        '''
        import transformers
        ...
        config = XverseConfig.from_hugging_face(hf_config_or_dir,
                                               dtype=dtype,
                                               mapping=mapping,
    ...
    def quantize(
        ...
        **kwargs,
        ...
        config = XverseConfig.from_hugging_face(hf_model_dir,
                                               dtype=dtype,
                                               mapping=mapping,
                                               quant_config=quant_config,
                                               **kwargs)
        print(f"Quantizing with quant_config: {config.quantization}")
